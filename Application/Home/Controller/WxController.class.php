...
class WxController extends Controller
{
    ...
    public function checkKeyword($key)
    {
        ...
        if ($key == "员工二维码") {
            ...
            $background = $this->createQrcodeBgEmp($employee);
        ...
        switch ($event) {
            ...
            case 'subscribe':
                ...
                if ($isold) {
                    ...
                    $tourl = self::$_url . '/App/Index/shop/ppid/' . $isold['id'] . '/';
                    ...
                    if ($user) {
                        ...
                        $tourl = self::$_url . '/App/Index/shop/ppid/' . $revip . '/';
                        $str = "<a href='" . $tourl . "'>" . htmlspecialchars_decode(self::$_set['wxsummary']) . "</a>";
                    } else {
                        $tourl = self::$_url . '/App/Index/shop/';
                        $str = "<a href='" . $tourl . "'>" . htmlspecialchars_decode(self::$_set['wxsummary']) . "</a>";
                    }
...
//            \Util\QRcode::png($url, './QRcode/qrcode/' . $openid . '.png', 'L', 6, 2);
    ...
    function createEmployeeQrcode($id, $openid)
    {
        ...
        if (!file_exists('./QRcode/qrcode/' . $id . "employee" . $openid . '.png')) {
            $url = 'http://' . $_SERVER['HTTP_HOST'] . __ROOT__ . '/App/Index/shop/ppid/' . $id;
    ...
    // 创建背景
    function createQrcodeBgEmp($employee)
    {
        if($employee['shop_id'] != 0){
            $autoset = M('Autoset')->where('shop_id=' . $employee("shop_id"))->find();
        }else{
            $autoset = M('Autoset')->where('id=1')->find();
        }
