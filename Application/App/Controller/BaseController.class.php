...
*/
        ...
        if ($_SESSION['sqmode'] && $_SESSION['sqopenid']) {
            ...
            if (strtolower(CONTROLLER_NAME) == 'shop') {
                ...
                if (strtolower(ACTION_NAME) == 'index') {
                    $footlasturl = base64_encode(U('App/Shop/index',array('shopid' => session("shop_id"))));
                    //临时处理不能分享的问题
        ...
        if (strtolower(CONTROLLER_NAME) == 'shop' && $_GET['ppid'] <> self::$WAP['vipid']) {
            // if (strtolower(ACTION_NAME) == 'index') {
            //     $this->redirect('App/Shop/index', array('shopid' => $_GET['shopid'],'ppid' => self::$WAP['vipid'], 'type' => I('type')));
            // }
...
//	}
    ...
    public function getPage($count, $psize, $loader, $loadername, $searchname, $map)
    {
        ...
        $page = new \Think\Page($count, $psize); // 实例化分页类 传入总记录数和每页显示的记录);

        $page->setConfig('header', '<li class="rows"></li>');
        $page->setConfig('prev', '上一页');
        $page->setConfig('next', '下一页');
        $page->setConfig('last', '末页');
        $page->setConfig('first', '首页');
        $page->setConfig('theme', '%FIRST%%UP_PAGE%%LINK_PAGE%%DOWN_PAGE%%END%%HEADER%');
        $page->lastSuffix = false;//最后一页不显示为总*/ }
