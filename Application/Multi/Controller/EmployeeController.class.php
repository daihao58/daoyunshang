...
class EmployeeController extends BaseController
{
    ...
    public function employeeList()
    {
        ...
        $bread = array(
            '0' => array(
                'name' => '员工列表',
                'url' => U('Multi/Employee/employeeList'),
            ),
        );
...
//        $m = M('employee');
    ...
    public function achievement()
    {
        ...
        $bread = array(
            '0' => array(
                'name' => '业绩统计',
                'url' => U('Multi/Employee/achievement'),
            ),
            '1' => array(
                'name' => '详情',
            ),
    ...
    public function vipAchievement()
    {
        ...
        $bread = array(
            ...
            '1' => array(
                ...
                'url' => U('Multi/Employee/vipAchievement', array('eid' => $eid)),
            ),
            '2' => array(
                'name' => '详情',
            ),
...
//更新推荐码
    public function employeetuijian(){
        if (IS_POST) {
            $data1['id']=I('id');
            if($data1['id'] ==''){
                $res['res']=0;
            ...
            $jiancha=M('employee')->where('id='.$data1['id'])->find();
            if(empty($jiancha)){
                $res['res']=0;
                $res['msg']='查询不到对应数据';
            }else{
                $data['tuijianma']=$this->yanzheng();
                M('employee')->where($data1)->save($data);
                $res['res']=1;
                $res['msg']='推荐码生成成功';
            }
            $this->ajaxreturn($res);
    ...
    private function yanzheng(){
        ...
        $chaxun=M('employee')->where('tuijianma="'.$code.'"')->find();
        if($chaxun){
            $this->yanzheng();
        }else{
            return $code;
