...
class IndexController extends BaseController
{
    ...
    public function main()
    {
        ...
        //汇总统计
        //总销售额
        $zxse = number_format($morder->where(array('ispay' => 1, 'shop_id' => session("homeShopId")))->sum('totalprice'), 2);
        //总成交额
        $zcje = number_format($morder->where(array('status' => 5, 'shop_id' => session("homeShopId")))->sum('totalprice'), 2);
        //总销量
        $zxl = $morder->where(array('ispay' => 1, 'shop_id' => session("homeShopId")))->sum('totalnum');
        //总佣金
