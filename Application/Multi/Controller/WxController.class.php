...
class WxController extends BaseController
{
    ...
    public function keywordSet()
    {
        ...
        $bread = array(
            ...
            '2' => array(
                'name' => '关键词设置',
                'url' => $id ? U('Multi/Wx/keywordSet', array('id' => $id)) : U('Multi/Wx/keywordSet')
            )
        );
        $this->assign('breadhtml', $this->getBread($bread));
        //处理POST提交
    ...
    public function img()
    {
        ...
        $bread = array(
            ...
            '1' => array(
                'name' => '关键词图文列表',
                'url' => U('Multi/Wx/img', array('kid' => $kid))
            )
        );
        $this->assign('breadhtml', $this->getBread($bread));
        //绑定搜索条件与分页
    ...
    public function imgSet()
    {
        ...
        $bread = array(
            ...
            '2' => array(
                'name' => '关键词图文设置',
                'url' => $id ? U('Multi/Wx/imgSet', array('id' => $id)) : U('Multi/Wx/imgSet')
            )
        );
        $this->assign('breadhtml', $this->getBread($bread));
        //处理POST提交
    ...
    public function qrcodeBgEmpSet()
    {
        ...
        if (IS_POST) {
            ...
            if ($_FILES['qrcode']['error'] > 0) {
                ...
                if (move_uploaded_file($_FILES['qrcode']['tmp_name'], $name)) {
                    M('autoset')->save(array('id' => I('post.id'), 'qrcode_emp_background' => $name));
                    echo "<script>parent.replaceok();</script>";
                } else {
                    echo $name . "asd";
                    echo "<script>parent.replaceFuck();</script>";
        ...
        $autoset = M('autoset')->where('shop_id=' . session("homeShopId"))->find();
        if (!$autoset) {
            echo "系统未配置";
        }
        $this->assign('img', __ROOT__ . '/' . $autoset['qrcode_emp_background']);
        $this->assign('id', $autoset['id']);
        $this->display();
    }
