...
class VipModel extends Model
{
    ...
    public function getChildren($id = 0)
    {
        ...
        foreach ($vips as $k => $vip) {
            $count = $this->where('pid='.$vip['id'].' and mobile is not null')->count();
            if ($count > 0) {
                $vips[$k]['type'] = 1;
                // 显示一级 数量
                $count = $this->where('pid='.$vip['id'].' and mobile is not null')->count();
                ...
                // 显示二级 数量
                $arr = array();
                $tmp = $this->field('id')->where('pid='.$vip['id'].' and mobile is not null')->select();
                foreach ($tmp as $v) {
                    array_push($arr, $v['id']);
                }
                $count = $this->where(array('pid' => array('in', in_parse_str($arr)),'mobile' => array('exp','is not null')))->count();
                if ($count <= 0) {
                    $count = 0;
                }
                $vips[$k]['count2'] = $count;
                // 显示三级 数量
                $arr = array();
                $tmp = $this->field('id')->where('pid='.$vip['id'].' and mobile is not null')->select(); // 一级
                foreach ($tmp as $v) {
                    array_push($arr, $v['id']);
                }
                $tmp2 = $this->field('id')->where(array('pid' => array('in', in_parse_str($arr)),'mobile' => array('exp','is not null')))->select(); // 二级
                $arr2 = array();
                foreach ($tmp2 as $v) {
                    array_push($arr2, $v['id']);
                }
                $count = $this->where(array('pid' => array('in', in_parse_str($arr2)),'mobile' => array('exp','is not null')))->count(); // 三级
                if ($count <= 0) {
                    $count = 0;
                }
                $vips[$k]['count3'] = $count;
            ...
            // 显示三级 数量
            // 自己的
            $vips[$k]['ocount'] = $morder->where(array('vipid' => $vip['id'], 'ispay' => 1,'mobile' => array('exp','is not null')))->count(); // 一级已支付
            $vips[$k]['osum'] = $morder->where(array('vipid' => $vip['id'], 'ispay' => 1,'mobile' => array('exp','is not null'), 'payprice' => array('gt', 0)))->sum('payprice'); // 一级支付总金额
            // 一级
            $arr = array();
            $tmp = $this->field('id')->where(array('pid' => $vip['id'],'mobile' => array('exp','is not null')))->select(); // 一级会员ID
            foreach ($tmp as $v) {
                array_push($arr, $v['id']);
            }
            $ocount = $morder->where(array('vipid' => array('in', in_parse_str($arr)), 'ispay' => 1,'mobile' => array('exp','is not null')))->count(); // 一级已支付
            $osum = $morder->where(array('vipid' => array('in', in_parse_str($arr)), 'ispay' => 1, 'payprice' => array('gt', 0),'mobile' => array('exp','is not null')))->sum('payprice'); // 一级支付总金额
            $vips[$k]['ocount1'] = $ocount;
            $vips[$k]['osum1'] = $osum;
            // 二级
            $tmp2 = $this->field('id')->where(array('pid' => array('in', in_parse_str($arr)),'mobile' => array('exp','is not null')))->select();
            $arr2 = array();
            foreach ($tmp2 as $v) {
                array_push($arr2, $v['id']);
            }
            $ocount = $morder->where(array('vipid' => array('in', in_parse_str($arr2)), 'ispay' => 1,'mobile' => array('exp','is not null')))->count();
            $osum = $morder->where(array('vipid' => array('in', in_parse_str($arr2)), 'ispay' => 1, 'mobile' => array('exp','is not null'),'payprice' => array('gt', 0)))->sum('payprice');
            $vips[$k]['ocount2'] = $ocount;
            $vips[$k]['osum2'] = $osum;
            // 三级
            $tmp3 = $this->field('id')->where(array('pid' => array('in', in_parse_str($arr2)),'mobile' => array('exp','is not null')))->select();
            $arr3 = array();
            foreach ($tmp3 as $v) {
                array_push($arr3, $v['id']);
            }
            $ocount = $morder->where(array('vipid' => array('in', in_parse_str($arr3)), 'ispay' => 1,'mobile' => array('exp','is not null')))->count();
            $osum = $morder->where(array('vipid' => array('in', in_parse_str($arr3)), 'ispay' => 1,'mobile' => array('exp','is not null'), 'payprice' => array('gt', 0)))->sum('payprice');
    ...
    {
        $vip = $this->where(array('id' => $id,'mobile' => array('exp','is not null')))->find();
        $employee = M('employee')->where(array('id' => $eid,'mobile' => array('exp','is not null')))->find();
        // 容错
        if (!($vip && $employee)) {
            return false;
        }
        ...
        $mark = true;
        do {
            $temp = $this->field('id')->where(array('id' => array('in', in_parse_str($tempp)),'mobile' => array('exp','is not null')))->select();
            foreach ($temp as $v) {
                array_push($tempp, $v['id'] + 0);
            }
            $temp = $this->field('id')->where(array('pid' => array('in', in_parse_str($tempp)),'mobile' => array('exp','is not null')))->select();
            foreach ($temp as $v) {
