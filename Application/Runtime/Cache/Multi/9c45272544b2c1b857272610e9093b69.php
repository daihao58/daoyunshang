		...
		<div class="widget">
			...
			<div class="widget-body">
				<div class="table-toolbar">
					...
					<?php if(empty($status)): ?><button href="javascript:void(0)" class="btn btn-primary" id="exportOrder1"><i class="fa fa-save"></i>导出订单</button><?php endif; ?>
					...
					<div class="pull-right">
						<form id="App-search">
							<label style="margin-bottom: 0px; width:400px;">
                                <div class="col-sm-7" style="width:auto;">
                                    <div class="input-group" >
                                        <div class="input-group-addon">
                                            <i class="fa fa-clock-o"></i>
                                        </div>
                                        <input type="text" class="form-control pull-right" name="timeRange" style="height:30px;"
                                               id="reservationtime" value="<?php echo ($timeRanges); ?>"placeholder="时间搜索">
                                    </div>
                                </div>
							</label>
							<label style="margin-bottom: 0px;">
								<input name="name" type="search" class="form-control input-sm" value="<?php echo ($name); ?>" placeholder="输入手机号搜索">
							</label>
							<label style="margin-bottom: 0px;">
								<input name="oid" type="search" class="form-control input-sm" value="<?php echo ($oid); ?>" placeholder="输入订单号搜索">
							</label>
							<a href="<?php echo U('Multi/Shop/order/');?>" class="btn btn-success" data-loader="App-loader" data-loadername="商城订单" data-search="App-search">
				...
				<table id="App-table" class="table table-bordered table-hover">
					...
					<?php case "7": ?>已退货<?php break; endswitch;?></td><?php endif; ?>
								...
								<td class="center ">
									<?php if(empty($status)): ?><a href="<?php echo U('Multi/Shop/orderDetail/',array('id'=>$vo['id'],'status'=>$status));?>" class="btn btn-primary btn-xs" data-loader="App-loader" data-loadername="订单详情"><i class="fa fa-eye"></i> 详情</a>
										<?php else: ?>
										<a href="<?php echo U('Multi/Shop/orderDetail/',array('id'=>$vo['id'],'status'=>$status));?>" class="btn btn-primary btn-xs" data-loader="App-loader" data-loadername="订单详情"><i class="fa fa-eye"></i> 详情</a>
...
<script type="text/javascript">
	...
	$('#App-delall').on('click',function(){
		...
		var toajax="<?php echo U('Multi/Shop/orderDel');?>"+"/id/"+chk;
		var callok=function(){
			//主框架可以直接模拟点击刷新按钮
			//$('#refresh-toggler').trigger('click');
...
var btnfhkd=$('.orderfhkd');
	$(btnfhkd).on('click',function(){
			...
			$.ajax({
					type:"post",
					url:"<?php echo U('Multi/Shop/orderFhkd');?>",
					data:{'id':id},
					dataType: "json",
	                	...
	                	buttons: {
	                		   success: {
			                        ...
			                        callback: function () {
			                        	...
			                        	$.ajax({
			                        		type:"post",
			                        		url:"<?php echo U('Multi/Shop/orderFhkdSave');?>",
			                        		data:dt,
			                        		dataType:"json",
			                        		async:false,
	...
	$(btnchange).on('click',function(){
			...
			$.ajax({
					type:"post",
					url:"<?php echo U('Multi/Shop/orderChange');?>",
					data:{'id':id},
					dataType: "json",
					//beforeSend:$.App.loading(),
					success:function(mb){
	                	...
	                	buttons: {
	                		   success: {
			                        ...
			                        callback: function () {
			                        	...
			                        	$.ajax({
			                        		type:"post",
			                        		url:"<?php echo U('Multi/Shop/orderChangeSave');?>",
			                        		data:dt,
			                        		dataType:"json",
			                        		async:false,
			                        		success:function(info){
	...
	$(btnclose).on('click',function(){
			...
			$.ajax({
					type:"post",
					url:"<?php echo U('Multi/Shop/orderClose');?>",
					data:{'id':id},
					dataType: "json",
					//beforeSend:$.App.loading(),
					success:function(mb){
	                	...
	                	buttons: {
	                		   success: {
			                        ...
			                        callback: function () {
			                        	...
			                        	$.ajax({
			                        		type:"post",
			                        		url:"<?php echo U('Multi/Shop/orderCloseSave');?>",
			                        		data:dt,
			                        		dataType:"json",
			                        		async:false,
			                        		success:function(info){
	...
	$(btndeliver).on('click',function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Multi/Shop/orderDeliver');?>",
			async:false,
			data:{'id':id},
			success:function(info){
			    if(info['status']){
	...
	$(btnchange).on('click',function(){
			...
			$.ajax({
					type:"post",
					url:"<?php echo U('Multi/Shop/orderTuihuo');?>",
					data:{'id':id},
					dataType: "json",
					//beforeSend:$.App.loading(),
					success:function(mb){
	                	...
	                	buttons: {
	                		   success: {
			                        ...
			                        callback: function () {
			                        	...
			                        	$.ajax({
			                        		type:"post",
			                        		url:"<?php echo U('Multi/Shop/orderTuihuoSave');?>",
			                        		data:dt,
			                        		dataType:"json",
			                        		async:false,
			                        		success:function(info){
	...
	$(btnsuccess).on('click',function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo U('Multi/Shop/ordersuccess');?>",
			async:false,
			data:{'id':id},
			success:function(info){
			    if(info['status']){
	...
	$('#exportOrder1').on('click',function(){
		...
		window.open("<?php echo U('Multi/Shop/orderExport');?>/status/-1/id/"+chk);
	})
</script>