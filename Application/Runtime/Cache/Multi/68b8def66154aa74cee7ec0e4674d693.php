<?php if (!defined('THINK_PATH')) exit();?><style>
	...
	#App-uploader-body .cover{ width: 130px; height: 100px; position: absolute; left: 0px; top:0px; background:url('/Public/Admin/img/choosed.png'); display: none;}
...
<div id="App-uploader">
	<div id="App-uploader-header">
		<div class="hide">
			...
			<form enctype="multipart/form-data" action="<?php echo U('Multi/Upload/doupimg');?>" method="post" id="App-uploader-form" target="App-uploader-frame" >
        ...
        <div class="input-group input-group-sm" style="margin-bottom: 20px;">
        		<input type="text" class="form-control" disabled value="" id="App-uploader-findback" placeholder="点击下方图片自动添加">
           		<span class="input-group-btn">
                	<button class="btn btn-default shiny" type="button" id="App-uploader-delall"><i class="glyphicon glyphicon-trash"></i>删除图片</button>
                </span>
        </div>
	</div>
	<div id="App-uploader-body" data-page = "2">
		<?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="imgwp" data-id = "<?php echo ($vo["id"]); ?>" data-check = "0" onclick="checkupload(this);">
...
<script>
	...
	$(Jupgetmore).on('click',function(){
		...
		var moreurl="<?php echo U('Multi/Upload/getmoreimg/');?>";
		var more;
		$.ajax({
			type:"post",			
			data:{'p':p},
