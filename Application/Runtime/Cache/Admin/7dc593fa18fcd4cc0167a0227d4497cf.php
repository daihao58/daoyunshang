                ...
                <table id="App-table" class="table table-bordered table-hover">
                    ...
                    <tbody>
                        <?php if(is_array($cache)): $i = 0; $__LIST__ = $cache;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id="item<?php echo ($vo["id"]); ?>">
                                ...
                                <td class=" ">
                                    <?php if(($vo["status"]) == "1"): ?><button class="btn btn-success btn-xs status" data-id="<?php echo ($vo["id"]); ?>" data-status="<?php echo ($vo["status"]); ?>" disabled>
                                            <i class="fa fa-arrow-up"></i>出售中</button>
                                        <?php else: ?>
                                        <button class="btn btn-danger btn-xs status" data-id="<?php echo ($vo["id"]); ?>" data-status="<?php echo ($vo["status"]); ?>" disabled>
                                            <i class="fa fa-arrow-down"></i>已下架</button><?php endif; ?>
                                </td>
                                <td class=" ">
                                ...
                                <td class="center ">
                                    <!--<a href="<?php echo U('Admin/Shop/goodsSet/',array('id'=>$vo['id']));?>" class="btn btn-success btn-xs" data-loader="App-loader" data-loadername="设置商品">-->
                                    <!--    <i class="fa fa-edit"></i> 管理-->
                                    <!--</a>&nbsp;&nbsp;-->
                                    <button class="btn btn-success btn-xs getlink" data-id="<?php echo ($vo["id"]); ?>">
                                        <i class="fa fa-link"></i> 商品链接
                                    </button>&nbsp;&nbsp;
                                    <a href="<?php echo U('Admin/Shop/goods/');?>" class="btn btn-danger btn-xs" data-type="del" data-ajax="<?php echo U('Admin/Shop/goodsDel/',array('id'=>$vo['id']));?>">
