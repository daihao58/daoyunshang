...
<head>
	...
	<link rel="stylesheet" href="/Public/App/ydy/css/style.css" />
	...
	<style>
		...
		.fengxiangwenjian{display:block;position:fixed;width:20px;height:20px;background: rgba(0,0,0,0.3); font-size:12px; color:#fff; padding:5px 10px; z-index:9999; right:10px; top:10px; border-radius:5px; }
		.fanhui{left:10px;}
	</style>
</head>
<body class="back1">
	...
	<?php if(($cache["issku"]) == "1"): ?><div id="sku-wrap">
			<?php if(is_array($skuinfo)): foreach($skuinfo as $key=>$vo): ?><div class="sku" data-attr="">
					...
					<div class="dtl-col ovflw">
						<?php if(is_array($vo["allitems"])): foreach($vo["allitems"] as $key=>$vo2): if(!empty($vo2["checked"])): ?><span data-attr = "<?php echo ($vo2["path"]); ?>" style="display: inline-block;margin-bottom: 8px"><?php echo ($vo2["name"]); ?></span><?php endif; endforeach; endif; ?>
					</div>
