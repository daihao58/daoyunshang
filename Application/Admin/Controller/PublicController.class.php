...
class PublicController extends BaseController
{
    ...
    public function login()
    {
        if (IS_POST) {
            ...
            $user = M('User')->where(array('username' => $data['username'], 'userpass' => md5($data['userpass']),'type'=>'1'))->find();
            $employee = M('employee')->where(array('username' => $data['username'], 'userpass' => md5($data['userpass']),'type'=>'1'))->find();
            if ($user) {
